...
**Source:** [Hierarchies of Memory Limited Computations](../works/hierarchies-of-memory-limited-computations.md) — the definitions of constructability preceding the two hierarchy theorems, which require a machine that stays within the bound while marking out exactly that bound, together with the surrounding discussion showing the constructable family contains the real-time countable functions, is closed under composition with the model's threshold function, and is therefore very rich above it. The same condition appears in its time-based form in [On the Computational Complexity of Algorithms](../works/on-the-computational-complexity-of-algorithms.md) — the definition of a countable bound as one a machine can tick out as it goes, the separation theorem stated only over such bounds, and the accompanying argument that every computable object admits one, which is how the restriction is shown not to cost coverage.
