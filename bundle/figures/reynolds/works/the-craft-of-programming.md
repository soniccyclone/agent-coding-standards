...
- [Two descriptions can admit exactly the same things and still disagree about the only property that matters](../lessons/two-descriptions-can-admit-the-same-things-and-still-differ-on-what-matters.md)
- [A structural cue the checker ignores is a channel where the reader and the machine can silently disagree](../lessons/a-cue-the-checker-ignores-can-lie-to-the-reader.md)
- [Ambiguity is a defect only when the readings differ in meaning; when they agree it is free generality](../lessons/ambiguity-is-a-defect-only-when-the-readings-differ-in-meaning.md)
- [Push the type discipline into the grammar, and the safety of inlining becomes a theorem about which production could have produced each hole](../lessons/index-the-grammar-by-type-so-substitution-safety-is-a-syntactic-theorem.md)
- [An operation that aborts on a condition the caller cannot test has no correct use](../lessons/an-untestable-precondition-makes-recovery-impossible.md)
- [Top-down refinement is not strictly top-down: name the part whose specification lives downstream and leave it empty](../lessons/name-the-part-whose-specification-lives-downstream-and-leave-it-empty.md)
- [Split on the domain before you split into steps, or partiality gets patched into whichever step discovers it](../lessons/split-on-the-domain-before-you-split-into-steps.md)
